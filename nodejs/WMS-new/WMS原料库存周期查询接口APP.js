...
async function SetData(body) {
    ...
    //先清空WMS原料仓库数据  PlanDeleteThingRecord(var objectID,DateTime startTime,DateTime endTime,Int16 timeRelation);
    var objectID = "3603812087760047849";
    var startTime = {
        "year": year - 1,
        "month": month,     /// 1-12
    ...
    if (!res.ret) {
        ...
        if (res.dataSets.length > 0) {
            ...
            if (Deleteret.ret == 0) {
                ...
                TraceInfo("开始添加记录 ,item个数", JSON.parse(body).Response.items.item.length);
                ...
                //添加记录 PlanAddThingRecord(objectID,recordValues,callback);
                TraceInfo("开始添加记录 ");
                var recordValues = [];
                recordValues= arr;
                ...
                if (addret.ret == 0) {
                    //业务处理
                    TraceInfo("添加wms原材料库存数据成功:", addret);
                } else {
                    TraceInfo("添加wms原材料库存数据失败:", addret);
                }        
            } else {
