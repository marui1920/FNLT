...
async function GetData() {
    ...
    if (returndata.ret == 0 && returndata.fieldValues.length > 0) {
        ...
        if (returndataManage.ret == 0 && returndataManage.dataSets.length == 1) {
            ...
            var product_model = returndataManage.dataSets[0].values[2].stringValue;// manage f3 产品编码
            TraceInfo("product_model----", product_model);
            var production_time = returndataManage.dataSets[0].values[17].stringValue;;// manage f18 生产日期
            TraceInfo("production_time----", production_time);
            var material_num = returndataManage.dataSets[0].values[15].stringValue; //manage f16 产品型号
            TraceInfo("material_num----", material_num);
            var barrel_number = returndata.fieldValues[0].values[2].stringValue;//fill f3 桶编号
            TraceInfo("barrel_number----", barrel_number);
            var gross_weight = returndata.fieldValues[0].values[7].stringValue;//fill f8 毛重
            TraceInfo("gross_weight----", gross_weight);
            var barrel_weight = returndata.fieldValues[0].values[8].stringValue;//fill f9 桶重
            TraceInfo("barrel_weight----", barrel_weight);
            var net_weight = returndata.fieldValues[0].values[9].stringValue;//fill f10 净重
            TraceInfo("net_weight----", net_weight);
            ...
            var supplierId = "SCCJ";
            var supplierName = "生产车间";
            var hedi09 = "1000";
            var lineNo = "1";
            var sku = product_model;
            var expectedQty = "1000";
            ...
            var dedi05 = barrel_number;
            var dedi04 = expectedQty;
