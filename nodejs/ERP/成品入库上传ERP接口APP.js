function main() {
    ...
    GetData().then(res => {
        if (JSON.stringify(res) !== "{}") {
            ...
            var sku = res.sku;
            var receivedQty = res.receivedQty;
            var stockinTime = res.stockinTime;
            var batchNumber = res.batchNumber;
            var warehouseERP = res.warehouseERP;
            var qualitySta = res.qualitySta;
            var barrelNumber = res.barrelNumber;
            var orderNumber = res.orderNumber;          
            var paramsData = {
                "method": "confirmASNData",
                        ...
                        "data": {
                            "orderinfo": [{
                                ...
                                "item": [{
                                    ...
                                    "SKU": '${sku}', 
                                    "ReceivedTime": '${stockinTime}', 
                                    "Lotatt01": "", 
                                    "Lotatt02": "", 
                                    "Lotatt03": "", 
                                    "Lotatt04": "", 
                                    "Lotatt05": '${batchNumber}', 
                                    "Lotatt06": "${warehouseERP}", 
                                    "Lotatt07": "", 
                                    "Lotatt08": "", 
                                    "Lotatt09": "", 
                                    "Lotatt10": "", 
...
async function GetData() {
    ...
    //11：产品编号，12：实收数量，14:入库日期，15：产品批号，17：ERP仓库，19：质量状态，21：收货桶号
    var fieldIDs = [11, 12, 14, 15, 17, 19, 21, 257];
    var startTime = {
        "year":year,
        "month":1,     /// 1-12
    ...
    if (!res.ret && res.dataSets.length !== 0) {
        ...
        data_obj.recordID = dataSets[0].values[7].uint64Value;
        data_obj.sku = dataSets[0].values[0].stringValue;
        data_obj.receivedQty = dataSets[0].values[1].stringValue;
        data_obj.stockinTime = dataSets[0].values[2].stringValue;
        data_obj.batchNumber = dataSets[0].values[3].stringValue;
        data_obj.warehouseERP = dataSets[0].values[4].stringValue;
        data_obj.qualitySta = dataSets[0].values[5].stringValue;
        data_obj.barrelNumber = dataSets[0].values[6].stringValue;
        var objectID1 = "87960930222082";
        //计划库工单管理生产查询
        //1：ERP订单号，2：MES工单号
        var fieldIDs1 = [1, 2];
        var filter1 = `f2=="${dataSets[0].values[3].stringValue}"`;//筛选出mes工单号对应的erp订单号
        ...
        if (!res1.ret && res1.dataSets.length !== 0) {
            ...
            data_obj.orderNumber = dataSets1[0].values[0].stringValue;           
        }
    }
    return data_obj;         
}
...
async function SetData(recordIDs) {
    ...
    for (let i = 0; i < recordIDs.length; i++) {
        obj = {};
        obj.keyValueList = [{"key":22,"value":{stringValue:"已上传"},}];
        arr.push(obj);
    }
