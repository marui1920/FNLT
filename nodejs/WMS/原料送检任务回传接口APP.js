...
function main() {
    ...
    //查询原材料质检回传、工单管理生产内符合条件数据，返回
    var date = intervalTime();
    var objectID = "54975581388807";
...
async function GetData(lens,dataSets) {
    ...
    for (var i = 0; i < lens; i++) {
        ...
        TraceInfo("WLDM:",WLDM);
        var objectID1 = "87960930284081";
        //4:产品编号;5:产品批号;6:实收数量;14:ERP组织;15:ERP仓库;16:质量状态;17:ERP货主;18:ASN编号;257:dataid
        var fieldIDs1 = [4, 5, 6, 14, 15, 16, 17, 18, 257];
        var filter1 = `(f4 == "${WLDM}")&&(f5 == "${batch_no}")`;
        var startTime = {
            "year":date.year,
            "month":date.month,     /// 1-12
            "day":1,       /// { 31,28/29,31,30,31,30,31,31,30,31,30,31 }
        ...
        var data1 = await PlanQueryThingRecord(objectID1,startTime,endTime,timeRelation,filter1,fieldIDs1).then(function (res) {
            ...
            data_obj.qtyOrdered = res.dataSets[0].values[2].stringValue;
            data_obj.lotAtt04 = res.dataSets[0].values[1].stringValue;
            ...
            data_obj.lotAtt07 = (res.dataSets[0].values[6].stringValue ? res.dataSets[0].values[6].stringValue : '' );
            data_obj.lotAtt08 = (res.dataSets[0].values[5].stringValue ? res.dataSets[0].values[5].stringValue : '' );
            data_obj.lotAtt13 = (res.dataSets[0].values[7].stringValue ? res.dataSets[0].values[7].stringValue : '' );
            data_obj.toLotAtt04 = res.dataSets[0].values[1].stringValue;
...
//将原材料质检回传内"待回传wms"的数据更新为"已下发wms"
