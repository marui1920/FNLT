class App {
    onCreate(appGuid) {
        ...
        app.post("/getWMS_CGRK", function (req, res) {
            TraceInfo("getWMS_CGRK","");
            const date = new Date();
            ...
            for (let i = 0; i < params.length; i++) {
                ...
                var orderType = params[i].orderType;//订单类型（CGRK：采购入库）
                ...
                for (let j = 0; j < params[i].details.length; j++) {
                    ...
                    var sku = params[i].details[j].sku;//产品编号
                    ...
                    var lotAtt13 = params[i].details[j].lotAtt13;//ASN 编号
                    var userDefine1 = params[i].details[j].userDefine1; //ERP 货主类型（ORG:组织，SUP:供应商，CUS:客户）
                    var userDefine2 = params[i].details[j].userDefine2; //产品名称
                    data_obj = {};
                    data_arr = [];
                    data_obj1 = {};
                    data_arr1 = [];  
                    ...
                    //区分采样地点
                    var location = '';
                    if (sku === "01030001" || sku === "01030002" || sku === "01030004" || sku.slice(0, 4) === "0102" ) {
                        location = "罐区";
                    } else {
                        location = "仓库";
                    }
                ...
                if ( !res1.ret ) {
                    ...
                    RealtimeAddThingRecord(objectID1,recordValues1,function (res2) {
                        if ( !res2.ret ) {
                            ...
                            //业务处理
                            TraceInfo("添加原材料质检任务下发及质检下发数据失败");
                            obj = {
                                "Response": {
                                    "return": {
                                        "returnCode": "0001",
        ...
        //绑定和侦听指定主机和端口上的连接
        var server = app.listen(30010, "172.16.24.205", function () {
